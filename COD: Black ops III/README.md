...
# VanityTS - Call of Duty: Black ops III
...
| `Bot's end games` |  Bots can't end the game, the score made by the bots will be ignored to prevent unespected endgame | ❌ |
...
## Usage
Download the files from the git and open the folder of the game you want to play with. After that select the folder or the folders inside and copy it into the correct folder. The mod can work on server side by putting the compiled gsc file into such as `BOIII Game Folder\BOIII\scripts\maps\mp\gametypes\` or for the `server BOIII UnraknedServer Folder\BOIII\scripts\maps\mp\gametypes\`
